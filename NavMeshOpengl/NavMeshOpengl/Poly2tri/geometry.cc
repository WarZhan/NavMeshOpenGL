...
#endif
	// debug zhanxi 2013/11/4 11:00
#if 1
	fstream ftxt;

// 	string myfile = _prefix + "_out.txt";
// 	ftxt.open(myfile.c_str(), ios::out);
	ftxt.open("test_in.txt", ios::out);

	Monopolys::iterator itms = _mpolys.begin();
	ftxt << _mpolys.size() << endl; //轮廓的个数
	for(; itms != _mpolys.end(); itms ++)
	{
...
bool Polygon::ChangeToConset(rcContourSet &cset)
{
	int maxContours;
	//sum = 0;
	maxContours = _mpolys.size(); //获取轮廓的数量
	//初始化轮廓集合
	...
	{		
		...
		for(; itm != itms->rend(); itm++)
		{
			...
			cont->verts[index ++] = 0;
			cont->verts[index ++] = y;
			cont->verts[index ++] = 131072; //用于标记 可忽略
		}
