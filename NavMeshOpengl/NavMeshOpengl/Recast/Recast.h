...
struct rcConfig
{
	...
	/// 跨度数小于这个值的任何地区，如果可能的话，与较大的地区合并。
	/// Any regions with a span count smaller than this value will, if possible, 
	/// be merged with larger regions. [Limit: >=0] [Units: vx] 
	int mergeRegionArea;
	
	/// 允许轮廓多边形转换过程期间产生的多边形的顶点的最大数目。
	...
	/// 最大距离的细节网格表面可能偏离的Heightfield数据。 （只适用于高度的细节。）
...
/// 代表一个简单的，非重叠的领域空间轮廓  顶点按逆时针顺序存放 
/// Represents a simple, non-overlapping contour in field space.
struct rcContour
{
	//简化轮廓顶点和连接数据 4*nverts
...
//包含的三角形网格，代表详细高度相关的数据与相关联的多边形网格对象的多边形。
